...
type GalleryItem = { id: string; url: string; prompt: string; model: string; createdAt: number; kind?: "image" | "video" };
type VideoJob = { id: string; runId: string; model: string; prompt: string; status: string; reference: string; error?: string; url?: string };
type CatalogModel = { id: string; name?: string; description?: string; capabilities?: Record<string, unknown>; supported_parameters?: Record<string, any> };
...
export default function Home() {
  ...
  const generateMutation = trpc.nano.generate.useMutation({
    onSuccess: (result: any) => {
      ...
      const items = data.map((item: any, index: number) => ({ id: `${Date.now()}-${index}`, url: getImageUrl(item), prompt, model: selectedModel?.name || modelId, createdAt: Date.now(), kind: "image" as const })).filter((item: GalleryItem) => item.url);
    ...
    const timer = window.setInterval(async () => {
      await Promise.all(pending.map(async job => {
        try {
          ...
          setVideoJobs(current => current.map(item => item.id === job.id ? { ...item, status, url: url || item.url } : item));
          if (url) {
            const galleryItem: GalleryItem = { id: job.id, url, prompt: job.prompt, model: job.model, createdAt: Date.now(), kind: "video" };
  ...
  const submitVideoBatch = async () => {
    ...
    try {
      ...
      const jobs = responses.map((result, index) => ({ id: `${Date.now()}-${index}`, runId: result?.runId || result?.id || result?.data?.runId || result?.data?.id, model: modelId, prompt, reference: refs[index], status: String(result?.status || result?.data?.status || "PENDING").toUpperCase() } as VideoJob)).filter(job => job.runId);
